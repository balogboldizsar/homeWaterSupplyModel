...
fprintf('\n K=[%5.3f,%5.3f,%5.3f];\n fval=%5.3f\n',K(1),K(2),K(3),fval); %K=0.0525 
end

function eSum=modell(K)
global p0 ro g
...
A1=D1^2*pi/4; Af=D2^2*pi/4; %Af=D2^2*pi/4; 
lambda=0.02; dzeta=3; %dzeta=1;

%kut jellemzoi
Hv=20; psz=ro*g*(L1-Hv);
...
% K=[Kp,0,0];
np=0;
TcPrev=0; Tm=0.001; %mintaveteli ido
...
eSum=sum(abs(eSout))/length(eSout);

Qstac=Qout(end,end);
pstac=pout(end,end);

fprintf('K=[%02.2f,%02.2f,%02.2f] -> ',K(1),K(2),K(3));
fprintf('n=%5.1f rpm, Qst=%3.0f l/min, pst=%6.0f Pa\n',n,Qstac*60e3,pstac);
end
...
global Af ro p0 A1 options pPrev
%global D1 dt a g lambda dzeta vPrev %nyomas vagy aramlas peremfeltetelhez
